<?php
session_start();
require 'config.php'; // pastikan $conn ada
...
if (isset($_GET['hapus'])) {
    ...
    if ($id_hapus > 0) {
        $sqlDel = "DELETE FROM tb_kategori_menu WHERE id_kategori = $id_hapus";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $id_edit  = (int) ($_POST['id_kategori'] ?? 0);

    if ($kategori === '') {
        $error = "Nama kategori wajib diisi.";
    } else {
        if ($aksi === 'tambah') {
            $stmt = mysqli_prepare($conn, "INSERT INTO tb_kategori_menu (kategori) VALUES (?)");
            ...
            $stmt = mysqli_prepare($conn, "UPDATE tb_kategori_menu SET kategori = ? WHERE id_kategori = ?");
            mysqli_stmt_bind_param($stmt, "si", $kategori, $id_edit);
...
if (isset($_GET['edit'])) {
    ...
    if ($id_edit_get > 0) {
        $sqlEdit = "SELECT * FROM tb_kategori_menu WHERE id_kategori = $id_edit_get LIMIT 1";
        $resEdit = mysqli_query($conn, $sqlEdit);

        if ($resEdit && mysqli_num_rows($resEdit) === 1) {
            $editKategori = mysqli_fetch_assoc($resEdit);
            $isEdit       = true;
        }
    }
}
...
// AMBIL DATA KATEGORI
$sqlList = "SELECT * FROM tb_kategori_menu ORDER BY id_kategori ASC";
...
<head>
    <meta charset="UTF-8">
    <title>Kategori Menu - Omah Kopi</title>
    ...
    <div class="main-content">
        ...
        <div class="row">
            ...
            <div class="col-md-7 mb-3">
                <div class="card">
                    <div class="card-header">
                        <strong>Daftar Kategori Menu</strong>
                    </div>
                    <div class="card-body">
                                ...
                                if ($result && mysqli_num_rows($result) > 0) {
                                    ...
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a href="kategori.php?edit=<?= $row['id_kategori']; ?>"
            ...
            <div class="col-md-5 mb-3">
                <div class="card">
                    <div class="card-header">
                        <strong><?= $isEdit ? 'Update Kategori' : 'Tambah Kategori'; ?></strong>
                    </div>
                    <div class="card-body">
                        <form action="kategori.php<?= $isEdit && $editKategori ? '?edit='.(int)$editKategori['id_kategori'] : ''; ?>" method="post">
                            <input type="hidden" name="aksi" value="<?= $isEdit ? 'update' : 'tambah'; ?>">

                            <?php if ($isEdit && $editKategori): ?>
                                <input type="hidden" name="id_kategori" value="<?= (int)$editKategori['id_kategori']; ?>">
                            <?php endif; ?>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text"
                                       ...
                                       value="<?= $isEdit && $editKategori ? htmlspecialchars($editKategori['kategori']) : ''; ?>"
                                       required>
                            </div>

...
// ambil data admin
$q = mysqli_query($conn, "SELECT * FROM tb_admin WHERE id_admin = $id_admin LIMIT 1");
$admin = mysqli_fetch_assoc($q);
if (!$admin) {
    die("Data staf/admin tidak ditemukan.");
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $name     = trim($_POST['name'] ?? '');
    $username = trim($_POST['username'] ?? '');
    $password = trim($_POST['password'] ?? ''); // boleh kosong (tidak diubah)
    ...
    if ($name === '' || $username === '') {
        ...
        $set = "name='$nameEsc', username='$userEsc'";

        if ($password !== '') {
            $passEsc = mysqli_real_escape_string($conn, $password); // masih plaintext
            $set .= ", password='$passEsc'";
        }

        $sqlUpdate = "UPDATE tb_admin SET $set WHERE id_admin = $id_admin";
        if (mysqli_query($conn, $sqlUpdate)) {
            $success = "Data staf/admin berhasil disimpan! Anda akan dialihkan...";
            // refresh data
            $q = mysqli_query($conn, "SELECT * FROM tb_admin WHERE id_admin = $id_admin LIMIT 1");
            $admin = mysqli_fetch_assoc($q);
        } else {
...
<head>
    <meta charset="UTF-8">
    <title>Edit Staf/Admin - Omah Kopi</title>
...
<div class="wrapper-admin">
    ...
    <div class="main-content">
        ...
        <div class="card mb-3">
            <div class="card-header">Form Edit Staf/Admin</div>
            <div class="card-body">
                <?php if (!empty($success)): ?>
                    <div class="alert alert-success"><?= htmlspecialchars($success); ?></div>
                    <script>
                        setTimeout(function () {
                            window.location.href = "staf_user.php?msg=updated";
                        }, 1500);
                    </script>
                <?php endif; ?>
                ...
                <form method="post">
                    ...
                    <a href="staf_user.php" class="btn btn-secondary">Kembali</a>
                </form>
            </div>
        </div>
    </div>
